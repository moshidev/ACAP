...
static double piFusion(int index, int size, int steps) {
	double partpi = 0.0;
	double num = index % 2 ? -1.0 : 1.0;
	double denom = 1.0 + index*2.0;
	double width = 1.0/steps;
	...
	return 4.0*partpi/2 + sum*width/2;
}
...
int main(int argc, char* argv[]){
	...
	if(argc!=2){	//El primer argumento siempre es el nombre del programa
		...
		if(steps<=0 && world_rank == 0){
            printf("num_iter,num_procs,leibniz_cput,leibniz_wallt,rectangles_cput,rectangles_wallt,fusion_cput,fusion_wallt,leibniz_pi,rectangles_pi,fusion_pi\n");
		}else{
			...
			struct resultado fusion = evalua(piFusion, steps, world_rank, num_procs);
            
			if (world_rank == MASTER_RANK) {
				printf("%d,%d,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf\n", steps, num_procs, leibniz.cput, leibniz.wallt, rectangulos.cput, rectangulos.wallt, fusion.cput, fusion.wallt, leibniz.pi, rectangulos.pi, fusion.pi);
			}
		}
	}
