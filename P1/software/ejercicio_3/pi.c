 ...
 * Ejercicio 3a de la primera práctica de ACAP.
 */
...
int main(int argc, char* argv[]){
	if(argc!=2){	//El primer argumento siempre es el nombre del programa
		...
		if(steps<=0){
            ...
            volatile double pi1 = piLeibniz(steps);
            cput1 = get_cpu_time() - cput1;
            wallt1 = get_wall_time() - wallt1;

            double cput2 = get_cpu_time();
            double wallt2 = get_wall_time();
            volatile double pi2 = piRectangles(steps);
            cput2 = get_cpu_time() - cput2;
            wallt2 = get_wall_time() - wallt2;

            printf("%d,%lf,%lf,%lf,%lf,%lf,%lf\n", steps, cput1, wallt1, cput2, wallt2, pi1, pi2);
		}
	}
	return 0;
}